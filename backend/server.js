...
// ── Motor command flag — set by frontend, consumed by ESP32 on next POST ─────
let motorCommandPending = false;

// ── Camera command flag — set by frontend, consumed by ESP32 on next upload ──
let cameraCommandPending = false;

// ── Irrigation command flag — set by /api/irrigate-all, consumed by ESP32 on next POST ──
...
app.post('/api/sensor-data', async (req, res) => {
  const { node_id, temperature, humidity, soil_moisture, soil_ph, lux } = req.body;
  ...
  try {
    ...
    await client.query(
      `INSERT INTO nodes (node_id, name, layer_number, temperature, humidity, soil_moisture, soil_ph, lux)
       ...
       ON CONFLICT (node_id) DO UPDATE SET
         ...
         updated_at    = NOW()`,
      [node_id, temperature ?? null, humidity ?? null, soil_moisture ?? null, soil_ph ?? null, lux ?? null]
    );

    // 2. Append to history
    const reading = await client.query(
      `INSERT INTO sensor_readings (node_id, temperature, humidity, soil_moisture, soil_ph, lux)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [node_id, temperature ?? null, humidity ?? null, soil_moisture ?? null, soil_ph ?? null, lux ?? null]
    );
    ...
    // Consume pending irrigation command (one-shot — clears after sending to ESP32)
    ...
    console.log(`[ESP32] ${node_id} → temp=${temperature} humi=${humidity} soil=${soil_moisture} ph=${soil_ph} lux=${lux}`);
    res.json({ status: 'ok', data: reading.rows[0], irrigate_pump: triggerPump });
...
app.put('/api/nodes/:id/sensors', requireAuth, async (req, res) => {
  const { id } = req.params;
  // App.jsx sends camelCase — map to snake_case DB columns
  const { temperature, humidity, soilMoisture, soilPH, lux } = req.body;
  try {
    const result = await pool.query(
      `UPDATE nodes SET
       ...
       WHERE id = $6
       RETURNING *`,
      [temperature ?? null, humidity ?? null, soilMoisture ?? null, soilPH ?? null, lux ?? null, id]
    );
    if (!result.rows[0]) return res.status(404).json({ message: 'Node not found' });
    ...
    await pool.query(
      `INSERT INTO sensor_readings (node_id, temperature, humidity, soil_moisture, soil_ph, lux)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [row.node_id, row.temperature, row.humidity, row.soil_moisture, row.soil_ph, row.lux]
    );
...
app.post('/api/nodes/:id/irrigate', requireAuth, async (req, res) => {
  ...
  try {
    await pool.query(
      `UPDATE nodes SET irrigation_status = 'active', updated_at = NOW() WHERE id = $1`,
      [id]
    );
    broadcast({ type: 'irrigation_start', nodeId: id, triggeredBy });

    await pool.query(
      `INSERT INTO irrigation_log (node_id, triggered_by) VALUES ($1, $2)`,
      [id, triggeredBy]
    );
 ...
 * 2. Sets irrigationCommandPending so the next ESP32 POST receives irrigate_pump:true.
 * 3. Auto-completes all nodes after 10 s (matches the frontend timer).
 */
app.post('/api/irrigate-all', requireAuth, async (req, res) => {
  const { triggeredBy = 'manual' } = req.body;
  try {
    // Mark all active nodes as irrigating
    const nodes = await pool.query(`SELECT id, node_id FROM nodes WHERE is_active = true`);
    ...
    // Signal the ESP32 to fire the pump on its next POST to /api/sensor-data
    irrigationCommandPending = true;

    broadcast({ type: 'irrigation_start', triggeredBy, scope: 'all' });

    console.log(`[IRRIGATE-ALL] Command issued — ${nodes.rows.length} node(s) marked active, pump flag set`);

    // Auto-complete after 10 s
    setTimeout(async () => {
      await pool.query(
        `UPDATE nodes SET irrigation_status = 'idle', last_irrigation = NOW(), updated_at = NOW()
         WHERE is_active = true`
      );
      broadcast({ type: 'irrigation_complete', scope: 'all' });
      console.log('[IRRIGATE-ALL] Cycle complete — all nodes reset to idle');
    }, 10000);

    res.json({ ok: true, nodesTriggered: nodes.rows.length });
  } catch (err) {
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
 ...
 * Polled by ESP32 every 2–3 s. Returns the pending flag and clears it (one-shot).
 * No auth — device endpoint, same as /api/camera/command.
 */
app.get('/api/irrigation/command', (req, res) => {
  ...
  res.json({ irrigate_pump: pending });
});

/**
 * POST /api/aflc/:id
 * App.jsx logs an AFLC decision every time nodeData changes.
 * Body: { decision, confidence, reason }
 */
app.post('/api/aflc/:id', requireAuth, async (req, res) => {
  const { id } = req.params;
  const { decision, confidence, reason } = req.body;
  try {
    await pool.query(
      `INSERT INTO aflc_decisions (node_id, decision, confidence, reason)
       VALUES ($1, $2, $3, $4)`,
...
//    water_present   → water_level_triggered  (INVERTED: present=true means OK,
...
app.post('/api/reservoir', async (req, res) => {
  ...
  // water_present=true  → reservoir has water  → NOT a low-water alert
  // water_present=false → reservoir is empty   → trigger the alert
  const water_level_triggered = water_present === undefined ? false : !water_present;

  try {
    // ── 1. Upsert live reservoir state (single row, id=1) ──────────────────
    await pool.query(
      `INSERT INTO reservoir (id, water_ph, tds, turbidity, water_level_triggered,
    ...
    // ── 4. Extra broadcast if water just went low ─────────────────────────
    if (water_level_triggered) {
      broadcast({ type: 'water_level_alert' });
    }

    console.log(
      `[ESP32-RESERVOIR] pH=${ph} (${ph_voltage}V) | TDS=${tds_ppm}ppm | ` +
      `Turbidity=${turbidity_ntu}NTU | Water=${water_present ? 'OK' : 'LOW!'} | Motor=${motor_state}`
    );

    // Consume the pending motor command (one-shot — clears after sending)
    ...
    res.json({ status: 'ok', motor_command: triggerMotor });
...
app.post('/api/reservoir/motor/run', requireAuth, (req, res) => {
  motorCommandPending = true;
  console.log('[MOTOR] Command queued — ESP32 will pick it up on next POST');
  res.json({ ok: true, message: 'Motor command queued for ESP32' });
});

// GET /api/reservoir  — polled by App.jsx every 8 s, requires auth
app.get('/api/reservoir', requireAuth, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM reservoir WHERE id = 1');
    if (!result.rows[0]) {
...
// GET /api/reservoir/log  — last N raw readings (for debugging / history panel)
app.get('/api/reservoir/log', requireAuth, async (req, res) => {
  const limit = parseInt(req.query.limit) || 50;
  try {
    const result = await pool.query(
      'SELECT * FROM reservoir_log ORDER BY recorded_at DESC LIMIT $1',
      [limit]
    );
    res.json(result.rows);
  } catch (err) {
...
//      → server reads flag, clears it, includes camera_command in response
//      → broadcasts camera_image over WebSocket so dashboard updates instantly
//   3. (Optional) ESP32 sees camera_command:true → captures again immediately
// ═════════════════════════════════════════════════════════════════════════════

// GET /api/camera/command — polled by ESP32 every few seconds to check for a
// pending capture. Lightweight — no image involved. Returns the flag and clears
// it so only one capture fires per button press.
app.get('/api/camera/command', (req, res) => {
  ...
  res.json({ camera_command: pending });
});
...
app.post('/api/camera/capture', requireAuth, (req, res) => {
  ...
  console.log('[CAMERA] Capture command queued — ESP32 will pick it up on next upload');
  res.json({ ok: true, message: 'Camera capture command queued for ESP32' });
});

// POST /api/camera/upload — called by ESP32-CAM; no auth token needed
app.post('/api/camera/upload', (req, res) => {
  const { image_base64, width, height, file_size } = req.body;
  ...
  // Broadcast the image to all connected React tabs immediately
  broadcast({
    type:      'camera_image',
  ...
  console.log(
    `[ESP32-CAM] Image received — ${width}×${height}px, ${file_size} bytes @ ${capturedAt}`
  );
  ...
  res.json({ status: 'ok', capturedAt, camera_command: triggerCapture });
});
...
app.get('/api/nodes/:id/history', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM sensor_readings WHERE node_id = (SELECT node_id FROM nodes WHERE id = $1)
...
app.get('/api/nodes/:id/irrigation-log', requireAuth, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM irrigation_log WHERE node_id = (SELECT node_id FROM nodes WHERE id = $1)
