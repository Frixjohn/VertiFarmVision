...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.error) {
      return (
        <div style={{ padding: 40, fontFamily: "monospace", color: "#f44336" }}>
          <h2>⚠ Dashboard crashed</h2>
          <pre style={{ whiteSpace: "pre-wrap", fontSize: 13 }}>{String(this.state.error)}</pre>
          <button onClick={() => window.location.reload()} style={{ marginTop: 16, padding: "8px 20px" }}>Reload</button>
        </div>
      );
    }
        ...
        >
          <defs>
            <linearGradient id={`g${s.id}`} x1="0" x2="1" y1="0" y2="1">
              <stop offset="0%" stopColor={theme === "dark" ? "#a487e0" : "#b6f0b0"} />
              <stop offset="60%" stopColor={theme === "dark" ? "#4a7a5c" : "#57b76a"} />
              <stop offset="100%" stopColor={theme === "dark" ? "#16241c" : "#1f7a3e"} />
            </linearGradient>
          </defs>
          <path
          ...
          <path
            ...
            stroke={theme === "dark" ? "#c084f530" : "#ffffff70"}
            strokeWidth="1.8"
            fill="none"
            opacity="0.3"
          />
            ...
            <div
              ...
              style={{
                ...
                background: theme === "dark"
                  ? `linear-gradient(180deg, ${isHovered ? "#c084f5" : "#a35cec"}, ${isHovered ? "#8b4fd9" : "#6f3aa8"})`
                  : `linear-gradient(180deg, ${isHovered ? "#c9a3f5" : "var(--leaf-2)"}, ${isHovered ? "#8b4fd9" : "var(--leaf-4)"})`,
              }}
...
function AppInner() {
  ...
  const showImageNotification = useCallback((message, imageData, capturedAt) => {
    const id = Date.now();
    const time = capturedAt
      ? new Date(capturedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })
      : new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    setNotificationLog(prev => [
      { id, message, type: 'image', time, imageData },
      ...prev.slice(0, 49),
    ]);
    // Also fire a short toast so user knows it arrived
      ...
      try {
        ...
        if (msg.type === "camera_image") {
          ...
          showImageNotification(
            `📷 Reservoir snapshot — ${msg.width}×${msg.height}px`,
            msg.imageData,
            msg.capturedAt
          );
        }
  ...
  const irrigateAll = useCallback(async () => {
    ...
    try {
      ...
      showToast("💧 Irrigation command sent to all nodes & ESP32!", "success");
    } catch (err) {
  ...
  const avgTemp     = useMemo(() => (((n1.temperature ?? 24.5) + (n2.temperature ?? 23.8)) / 2).toFixed(1), [n1, n2]);
  const avgHumidity = useMemo(() => Math.round(((n1.humidity ?? 65) + (n2.humidity ?? 68)) / 2), [n1, n2]);
  const avgCO2      = useMemo(() => Math.round(((n1.co2 ?? 800) + (n2.co2 ?? 820)) / 2), [n1, n2]);
  const avgLux      = useMemo(() => Math.round(((n1.lux ?? 0) + (n2.lux ?? 0)) / 2), [n1, n2]);

  const temperatureChart = useMemo(() => [
    { label: "Node 1", value: n1.temperature ?? 24.5, unit: "°C" },
    { label: "Node 2", value: n2.temperature ?? 23.8, unit: "°C" },
  ], [n1, n2]);

  const humidityChart = useMemo(() => [
    { label: "Node 1", value: n1.humidity ?? 65, unit: "%" },
    { label: "Node 2", value: n2.humidity ?? 68, unit: "%" },
  ], [n1, n2]);

  const co2Chart = useMemo(() => [
    { label: "Node 1", value: n1.co2 ?? 800, unit: " ppm" },
    { label: "Node 2", value: n2.co2 ?? 820, unit: " ppm" },
  ], [n1, n2]);

  const luxChart = useMemo(() => [
    { label: "Node 1", value: Math.round(n1.lux ?? 0), unit: " lx" },
    { label: "Node 2", value: Math.round(n2.lux ?? 0), unit: " lx" },
  ], [n1, n2]);

  const aflcNode1 = useMemo(() => calculateAFLCDecision(n1), [n1]);
  const aflcNode2 = useMemo(() => calculateAFLCDecision(n2), [n2]);
      ...
      <main className="main">
                  ...
                  <div className="notif-list">
                    {notificationLog.length === 0 ? (
                      ...
                      notificationLog.map(n => (
                        <div key={n.id} className={`notif-item notif-item-${n.type === 'image' ? 'success' : n.type}`}>
                          <span className="notif-icon">
                            {n.type === "image" ? "📷" : n.type === "success" ? "✅" : n.type === "error" ? "❌" : n.type === "warning" ? "⚠️" : "ℹ️"}
                          </span>
                          <div className="notif-body">
                            ...
                            {n.imageData && (
                              <img
                                ...
                                onClick={() => window.open(n.imageData, '_blank')}
            ...
            <section className="reservoir-strip">
              ...
              <div className="reservoir-strip-grid">
                ...
                <div className="res-strip-item">
                  ...
                  <div>
                    <div className="res-strip-label">stepper Motor</div>
                ...
                <div className="res-strip-item" style={{ gridColumn: '1 / -1', justifyContent: 'center', paddingTop: '4px', gap: '10px' }}>
                  <button
                  ...
                  <button
                    ...
                    style={{
                      padding: '8px 20px',
                      background: cameraCapturing ? '#555' : '#2e7d32',
                      color: '#fff',
                      border: 'none',
                      borderRadius: '8px',
                      fontWeight: 'bold',
                      cursor: cameraCapturing ? 'not-allowed' : 'pointer',
                      opacity: cameraCapturing ? 0.6 : 1,
                      transition: 'background 0.2s',
                    }}
                  >
                    {cameraCapturing ? '⏳ Capturing…' : '📷 Capture Image'}
                  </button>
                </div>
              </div>
            </section>
              ...
              <div className="aflc-grid">
                  ...
                  <div className="aflc-body">
                    ...
                    <div className="aflc-metrics">
                      <div className="metric-item">
                        <span>CO₂: {n1.co2} ppm</span>
                        <span className={n1.co2 >= 400 && n1.co2 <= 1200 ? "status-ok" : "status-warn"}>
                          {n1.co2 >= 400 && n1.co2 <= 1200 ? "✓" : "⚠"}
                        </span>
                      </div>
                      <div className="metric-item">
                        <span>Temp: {n1.temperature}°C</span>
                        <span className={n1.temperature >= 22 && n1.temperature <= 26 ? "status-ok" : "status-warn"}>
                          {n1.temperature >= 22 && n1.temperature <= 26 ? "✓" : "⚠"}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="aflc-card">
                  ...
                  <div className="aflc-body">
                    ...
                    <div className="aflc-metrics">
                      <div className="metric-item">
                        <span>CO₂: {n2.co2} ppm</span>
                        <span className={n2.co2 >= 400 && n2.co2 <= 1200 ? "status-ok" : "status-warn"}>
                          {n2.co2 >= 400 && n2.co2 <= 1200 ? "✓" : "⚠"}
                        </span>
                      </div>
                      <div className="metric-item">
                        <span>Temp: {n2.temperature}°C</span>
                        <span className={n2.temperature >= 22 && n2.temperature <= 26 ? "status-ok" : "status-warn"}>
                          {n2.temperature >= 22 && n2.temperature <= 26 ? "✓" : "⚠"}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <section className="charts-grid">
              <div className="card big-card">
                ...
                <div className="node-summary">
                  <div className="summary-item"><span>🌡️ Temperature:</span><strong>{n1.temperature}°C</strong></div>
                  <div className="summary-item"><span>💧 Humidity:</span><strong>{n1.humidity}%</strong></div>
                  <div className="summary-item"><span>🫧 CO₂:</span><strong>{n1.co2} ppm</strong></div>
                  <div className="summary-item"><span>☀️ Light:</span><strong>{Math.round(n1.lux)} lx</strong></div>
                </div>
              </div>
              <div className="card big-card">
                <div className="card-top">
                ...
                <div className="node-summary">
                  <div className="summary-item"><span>🌡️ Temperature:</span><strong>{n2.temperature}°C</strong></div>
                  <div className="summary-item"><span>💧 Humidity:</span><strong>{n2.humidity}%</strong></div>
                  <div className="summary-item"><span>🫧 CO₂:</span><strong>{n2.co2} ppm</strong></div>
                  <div className="summary-item"><span>☀️ Light:</span><strong>{Math.round(n2.lux)} lx</strong></div>
                </div>
              </div>
            </section>
          </>
            ...
            <div className="sensor-grid">
              <div className="sensor-card">
                <div className="sensor-header">
                  ...
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node1", key: "temperature", name: "Temperature (Node 1)", value: n1.temperature, unit: "°C", min: 15, max: 35, step: 0.1 })}>✏️</button>
                </div>
                <div className="sensor-value">{n1.temperature}°C</div>
                <div className="sensor-status" style={{ color: n1.temperature >= 22 && n1.temperature <= 26 ? "#4caf50" : "#ff9800" }}>
                  {n1.temperature >= 22 && n1.temperature <= 26 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                  <h4>Node 1 - Humidity</h4>
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node1", key: "humidity", name: "Humidity (Node 1)", value: n1.humidity, unit: "%", min: 40, max: 90, step: 1 })}>✏️</button>
                </div>
                <div className="sensor-value">{n1.humidity}%</div>
                <div className="sensor-status" style={{ color: n1.humidity >= 60 && n1.humidity <= 75 ? "#4caf50" : "#ff9800" }}>
                  {n1.humidity >= 60 && n1.humidity <= 75 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                  <h4>Node 1 - CO₂</h4>
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node1", key: "co2", name: "CO₂ (Node 1)", value: n1.co2, unit: " ppm", min: 300, max: 3000, step: 10 })}>✏️</button>
                </div>
                <div className="sensor-value">{n1.co2} ppm</div>
                <div className="sensor-meta muted tiny">Sensirion SCD4x</div>
                <div className="sensor-status" style={{ color: n1.co2 >= 400 && n1.co2 <= 1200 ? "#4caf50" : "#ff9800" }}>
                  {n1.co2 >= 400 && n1.co2 <= 1200 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                ...
                <div className="sensor-value">{Math.round(n1.lux)} lx</div>
                <div className="sensor-status" style={{ color: n1.lux >= 5000 && n1.lux <= 80000 ? "#4caf50" : "#ff9800" }}>
                  {n1.lux >= 5000 && n1.lux <= 80000 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
            </div>
            ...
            <div className="sensor-grid">
              <div className="sensor-card">
                <div className="sensor-header">
                  ...
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node2", key: "temperature", name: "Temperature (Node 2)", value: n2.temperature, unit: "°C", min: 15, max: 35, step: 0.1 })}>✏️</button>
                </div>
                <div className="sensor-value">{n2.temperature}°C</div>
                <div className="sensor-status" style={{ color: n2.temperature >= 22 && n2.temperature <= 26 ? "#4caf50" : "#ff9800" }}>
                  {n2.temperature >= 22 && n2.temperature <= 26 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                  <h4>Node 2 - Humidity</h4>
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node2", key: "humidity", name: "Humidity (Node 2)", value: n2.humidity, unit: "%", min: 40, max: 90, step: 1 })}>✏️</button>
                </div>
                <div className="sensor-value">{n2.humidity}%</div>
                <div className="sensor-status" style={{ color: n2.humidity >= 60 && n2.humidity <= 75 ? "#4caf50" : "#ff9800" }}>
                  {n2.humidity >= 60 && n2.humidity <= 75 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                  <h4>Node 2 - CO₂</h4>
                  <button className="btn-edit" onClick={() => setEditingSensor({ node: "node2", key: "co2", name: "CO₂ (Node 2)", value: n2.co2, unit: " ppm", min: 300, max: 3000, step: 10 })}>✏️</button>
                </div>
                <div className="sensor-value">{n2.co2} ppm</div>
                <div className="sensor-meta muted tiny">Sensirion SCD4x</div>
                <div className="sensor-status" style={{ color: n2.co2 >= 400 && n2.co2 <= 1200 ? "#4caf50" : "#ff9800" }}>
                  {n2.co2 >= 400 && n2.co2 <= 1200 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
              <div className="sensor-card">
                <div className="sensor-header">
                ...
                <div className="sensor-value">{Math.round(n2.lux)} lx</div>
                <div className="sensor-status" style={{ color: n2.lux >= 5000 && n2.lux <= 80000 ? "#4caf50" : "#ff9800" }}>
                  {n2.lux >= 5000 && n2.lux <= 80000 ? "✓ Optimal" : "⚠ Check Range"}
                </div>
              </div>
            </div>
        ...
        {activePage === "Nodes" && (
          <section className="nodes">
            ...
            {/* ── Single system-wide irrigation button ── */}
            <div style={{
              background: irrigateAllStatus === "active"
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: "#c8e6c9" }}>
                  {irrigateAllStatus === "active"
                    ? "🌊 Irrigating all nodes… pump signal sent to ESP32"
                    : "Triggers all active nodes + sends pump signal to ESP32 simultaneously"}
                </div>
              </div>
              <button
                className="btn-irrigate"
                style={{
                  fontSize: 16,
                  padding: "14px 32px",
                  background: irrigateAllStatus === "active" ? "#81c784" : "#fff",
                  color: irrigateAllStatus === "active" ? "#1b5e20" : "#2e7d32",
                  fontWeight: 700,
                  minWidth: 200,
                  opacity: irrigateAllStatus === "active" ? 0.85 : 1,
                  cursor: irrigateAllStatus === "active" ? "not-allowed" : "pointer",
                }}
                onClick={irrigateAll}
                disabled={irrigateAllStatus === "active"}
              >
                {irrigateAllStatus === "active" ? "⏳ Irrigating…" : "💧 Irrigate Entire System"}
              </button>
            </div>
            ...
            <div className="node-cards">
              <div className="node-detail-card">
                ...
                <div className="node-detail-body">
                  <div className="detail-row"><span className="detail-label">🌡️ Temperature:</span><span className="detail-value">{n1.temperature}°C</span></div>
                  <div className="detail-row"><span className="detail-label">💧 Humidity:</span><span className="detail-value">{n1.humidity}%</span></div>
                  <div className="detail-row"><span className="detail-label">🫧 CO₂:</span><span className="detail-value">{n1.co2} ppm</span></div>
                  <div className="detail-row"><span className="detail-label">☀️ Light:</span><span className="detail-value">{Math.round(n1.lux)} lx</span></div>
                  <div className="detail-row">
                    <span className="detail-label">🕒 Last Irrigation:</span>
                    <span className="detail-value">{n1.lastIrrigation}</span>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">Status:</span>
                    <span className={`irrigation-badge ${irrigateAllStatus === "active" ? "active" : n1.irrigationStatus}`}>
                      {irrigateAllStatus === "active" || n1.irrigationStatus === "active" ? "💧 Irrigating..." : "✓ Idle"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="node-detail-card">
                ...
                <div className="node-detail-body">
                  <div className="detail-row"><span className="detail-label">🌡️ Temperature:</span><span className="detail-value">{n2.temperature}°C</span></div>
                  <div className="detail-row"><span className="detail-label">💧 Humidity:</span><span className="detail-value">{n2.humidity}%</span></div>
                  <div className="detail-row"><span className="detail-label">🫧 CO₂:</span><span className="detail-value">{n2.co2} ppm</span></div>
                  <div className="detail-row"><span className="detail-label">☀️ Light:</span><span className="detail-value">{Math.round(n2.lux)} lx</span></div>
                  <div className="detail-row">
                    <span className="detail-label">🕒 Last Irrigation:</span>
                    <span className="detail-value">{n2.lastIrrigation}</span>
                  </div>
                  <div className="detail-row">
                    <span className="detail-label">Status:</span>
                    <span className={`irrigation-badge ${irrigateAllStatus === "active" ? "active" : n2.irrigationStatus}`}>
                      {irrigateAllStatus === "active" || n2.irrigationStatus === "active" ? "💧 Irrigating..." : "✓ Idle"}
                    </span>
                  </div>
                </div>
              </div>
